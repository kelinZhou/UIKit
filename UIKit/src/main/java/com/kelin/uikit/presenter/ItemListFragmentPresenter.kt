...
abstract class ItemListFragmentPresenter<V : ItemListDelegate<VC, I>, VC : ItemListDelegate.ItemListDelegateCallback<I>, ID, I : Cell, W> : CommonListWrapperFragmentPresenter<V, VC, ID, I, W, List<I>>() {
    ...
    final override fun onListItemChildClick(itemView: View, position: Int, v: View, item: I) {
        if (!onInterceptListItemChildClick(itemView, position, v, item)) {
     ...
     * @return 如果需要拦截不交给Item处理则返回true，否则返回false。
     */
    protected open fun onInterceptListItemChildClick(iv: View, position: Int, v: View, item: I): Boolean {
        return false
    }
