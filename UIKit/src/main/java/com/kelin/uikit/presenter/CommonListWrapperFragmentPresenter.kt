...
import com.kelin.uikit.delegate.CommonListWrapperDelegate
import io.reactivex.Observable


abstract class CommonListWrapperFragmentPresenter<V : CommonListWrapperDelegate<VC, I, VD>, VC : CommonListWrapperDelegate.CommonListDelegateCallback<I>, ID, I, W, VD> : CommonFragmentPresenter<V, VC, ID, W, VD>() {
    ...
    override val initialDataProxy: IdActionDataProxy<ID, W> by lazy {
        if (isEnablePage) {
            ...
            ProxyFactory.createIdActionProxy {
    ...
    override val actionParameter: PageActionParameter by lazy {
        PageActionParameter.createInstance(
            isEnablePage
        )
    }
    ...
    override fun onLoadInitData(id: ID?, loadAction: LoadAction) {
    ...
    override fun onRefreshData(showProgress: Boolean, action: LoadAction) {
        if (setupMode != SETUP_NO_PROXY) {
            if (showProgress) {
     ...
     * 数据请求回调
     */
    protected inner class CommonDataListCallbackImpl<ACTION : ActionParameter> :
        CommonDataLoadCallbackImpl<ACTION>(),
        IdActionDataProxy.PageIdDataCallback<ID, ACTION, W> {

        override fun onFinished(id: ID, action: ACTION) {
        ...
        override fun onSuccess(id: ID, action: ACTION, data: W) {
            ...
            if (action.action == LoadAction.REFRESH || action.action == LoadAction.AUTO_REFRESH) {
                ...
                if (isEnablePage && action.action == LoadAction.AUTO_REFRESH && action is PageActionParameter) {
                    action.pages?.let {
                        it.page =
                            (it.size + PageActionParameter.DEFAULT_PAGE_SIZE - 1) / PageActionParameter.DEFAULT_PAGE_SIZE
    ...
    protected open fun canShowNoMoreDataStyle(): Boolean {
        return (initialData as? Collection<*>)?.let { it.size >= actionParameter.pages?.size ?: 20 }
            ?: false
    }
    ...
    open inner class CommonListDelegateCallbackImpl : CommonDelegateCallbackImpl(),
        ...
        override val canShowNoMoreStyle: Boolean
            get() = this@CommonListWrapperFragmentPresenter.canShowNoMoreDataStyle()
        ...
        override fun onListItemChildClick(itemView: View, position: Int, v: View, item: I) {
            this@CommonListWrapperFragmentPresenter.onListItemChildClick(
                itemView,
                position,
                v,
                item
