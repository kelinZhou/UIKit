...
abstract class SuperRecyclerViewAdapter<T, VH>(
    ...
    private val noMoreDataHandler: NoMoreDataHandler,
    ...
    private fun startLoadMore() {
        Logger.system(javaClass.simpleName)?.i("开始加载更多")
    ...
    private fun reloadMore() {
        Logger.system(javaClass.simpleName)?.i("重新开始加载更多")
        onLoadMoreListener?.run {
            if (invoke(true)) {
                lM?.isInTheLoadMore = true
            }
    ...
    fun setLoadMoreFinish() {
        Logger.system(javaClass.simpleName)?.i("加载完成")
    ...
    fun setLoadMoreFailed() {
        Logger.system(javaClass.simpleName)?.i("加载失败")
        lM?.isInTheLoadMore = false
        lM?.setRetryState()
    }
    ...
    fun setNoMoreData(noMoreData: Boolean) {
        if (noMoreData) {
            Logger.system(javaClass.simpleName)?.i("已加载到最后一页，加载完成")
            lM?.isNoMoreState = !noMoreDataHandler.onShowNoMore()
        } else {
            lM?.isLoadState = true
        }
    }
    ...
    open fun addItem(position: Int, itemData: Collection<T>, refresh: Boolean = true) {
    ...
    fun notifyItem(itemData: T) {
        val position = getItemPosition(itemData)
    ...
    fun getItemOrNull(position: Int): T? {
        return if (position < 0 || position > dataList.lastIndex) {
            null
        } else {
            getItem(position)
        }
